...
```
location /api {
        proxy_pass http://your vps-ip:8800;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
...
```
```
mkdir /var/www/project/client
```

```
cp -r build/* /var/www/prohect/client
```

Let's make some server configuration
```
 location / {
        root /var/www/project/client/;
        index  index.html index.htm;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
...
server {
  listen 80;
  server_name admin.your-domain name;
  location / {
    root /var/www/project/admin;
    index  index.html index.htm;
    proxy_http_version 1.1;
    proxy_set_header Upgrade $http_upgrade;
    proxy_set_header Connection 'upgrade';
